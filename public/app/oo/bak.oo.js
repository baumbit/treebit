const OO = function(rootElement, store={}, context={}, ooptions={}, pAArent) { //console.log('OO');
    ...
    if(!pAArent) {
        ...
        context.router = (function() {
            ...
            function goTo(path='', title, hints={}, other) {
                ...
                const routerBasename = ooptions.routerBasename;
                if(hints.reload) {
                    // recycle data
                ...
                //console.log({baseUrl, stateUrl, routePath, path, routeUrl, route, hints});
            }
        ...
        context.OO = function(paarent, Tug, x_prop_str, props={}, isSetup, debugTag) {
           ...
           //console.log({elementName, isFunction, Tug, x_prop_str, props});
            ...
            f.onclick = (cb, options={capture:true}) => {
                const ff = (event) => {
                    const v = cb({event, oo: f.oo, $: f.$});
                    if(v === false) event.stopPropagation();
                    return v;
                };
                f.onRemoved.push(() => { elm.removeEventListener('mouseup', ff, false); });
                elm.addEventListener('mouseup', ff, options);
                return f;
            };
            f.onevent = (eventType, cb) => {        // note: does not add async to promises, because its an event
                const ff = (event) => { //console.log('eventHandler', eventType, {...event});
                    //const v = cb({event, oo: f.oo, $: f.$});
                    const v = cb({...f.oo, event, value:elm.value});
                    if(v === false) event.stopPropagation();
                    return v;
                };
                f.onRemoved.push(() => { elm.removeEventListener(eventType, ff, false); });
                elm.addEventListener(eventType, ff);
                return f;
            };
            f.oninput = (cb) => {
                //f.onevent('input', ({event, oo, $}) => cb({event, oo, $, value:oo.elm.value}));
                f.onevent('input', ({event, oo, $}) => cb({...oo, event, value:elm.value}));
                return f;
            };
...
OO.swipe = function(cb/*y, flingSpeed*/, oo, xController, yController, {dragThresholdX=3, dragThresholdY=3, speedFactorX=5, speedFactorY=10, pauseThresholdMs=100}) {
    ...
    let elm = oo.elm,
        ...
        down = (event) => { //console.log('swipe DOWN');
        ...
        move = (event) => { //console.log('swipe MOVE');
            if(!isSwiping) return;
            if(oo.isDestroyed) return;
        ...
        up = (event) => {        //                                                       console.log('swipe UP');
            if(!isSwiping) return;
            if(oo.isDestroyed) return;
            isSwiping = false;
            if(xController) upSet(dataX, event.screenX, dragThresholdX, speedFactorX);
            if(yController) upSet(dataY, event.screenY, dragThresholdY, speedFactorY); //console.log({dataY});
